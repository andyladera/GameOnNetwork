...
class ChatModel {
    ...
    public function enviarSolicitudAmistad($solicitante_id, $receptor_id) {
        ...
        $sql = "INSERT INTO amistades (usuario_solicitante_id, usuario_receptor_id, estado) VALUES (?, ?, 'pendiente')";
        $stmt = $this->conn->prepare($sql);
    ...
    public function obtenerSolicitudesPendientes($usuario_id) {
        $sql = "SELECT a.id, a.usuario_solicitante_id, u.nombre, u.apellidos, u.username, a.fecha_solicitud
                FROM amistades a
    ...
    public function buscarUsuarios($busqueda, $usuario_actual_id) {
        ...
        $sql = "SELECT id, nombre, apellidos, username 
                ...
                LIMIT 10";
        $stmt = $this->conn->prepare($sql);
        $stmt->bind_param("sssi", $busqueda, $busqueda, $busqueda, $usuario_actual_id);
        $stmt->execute();
        return $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
    }
    ...
    public function obtenerEquiposUsuario($usuario_id) {
        $sql = "SELECT e.id, e.nombre, e.descripcion, d.nombre as deporte, 
                       em.rol, e.creado_en, 
